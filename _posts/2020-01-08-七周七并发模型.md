...
**隐藏的可变状态**<br />很多我们以为线程安全的类实际并不是，在并发编程中隐藏和逃逸仅仅是两种可变状态带来的风险。<br />书中提到的SimpleDateFormat例子，在阿里的Java开发规约中是有提到的。除了加锁、使用DateUtils工具外，同时也可以使用ThreadLocal来处理。
...
#### Day1
Lambda架构是站在大规模数据场景来解决问题的，不但解决了规模庞大的问题，还可以构建出对硬件错误和人为错误进行容错的系统。Lambda架构中两个主要的组件是：批处理层与加速层。
